...
var foounit = (function (){
  ...
  var sniff = function (ns){
    try {
      ...
      safeset(ns, 'onload', function (func){ func.apply(foounit.global, []); });
      return;
    } catch (e){}
    ...
    safeset(ns, 'onload', function (func){ foounit.global.onload = func; });
  }

  var setupConsole = function (){
    // Set up console
    var g = foounit.global;
    if (g.console){ return; }
  ...
  sniff(foounit);
  setupConsole();
  safeset(foounit, 'require');
  safeset(foounit, 'mixin', mixin);
  ...
  var deepEquality = function (actual, expected){
    ...
    // FIXME: The implicit cast should not work if we arent counting on 1 === true
      ...
      //foounit.puts('3: ' + actual + ', ' + expected);
      return actual == expected;
    }
  }

  safeset(foounit, 'matchers', {
    equal: {
      ...
      , match: function(actual, expected){
        //foounit.puts('deepEquality(actual, expected): ' + deepEquality(actual, expected));
  ...
  // foo unit keywords
  safeset(foounit, 'keywords', [foounit.matchers], {
    describe: function (description, func){
      var group = new foounit.ExampleGroup(this.getCurrentGroup(), description);
    ...
    , it: function (description, func){
      var example = new foounit.Example(this.getCurrentGroup(), description, func); 
      this.getCurrentGroup().addExample(example, func);
    }

    , xit: function (description, func){
      var example = new foounit.Example(this.getCurrentGroup(), description, func, true); 
      this.getCurrentGroup().addExample(example, func);
    }
  ...
  safeset(foounit, 'Expectation', function (actual){
    this._actual = actual;
  ...
  safeset(foounit, 'RunContext', function (){
  });

  // TODO: Add events handling for pass, fail and complete.
  safeset(foounit, 'Runner', function (example, level){
    ...
    this.run = function (){
      ...
      var context = new foounit.RunContext();
  ...
  safeset(foounit, 'ExampleGroup', function(parentGroup, description){
    this._groups = [];
    this._examples = [];
    this._before = null;
    this._after  = null;
  ...
  safeset(foounit, 'Context', [foounit.keywords], function (group){
  ...
  safeset(foounit, 'Example', function (parentGroup, description, func, pending){
    this._parent = parentGroup;
  ...
  mixin(foounit, {
    _root: new foounit.ExampleGroup()

    // NOTE: When you want to add a root level test use add
    , add: function (func){
      var group = new foounit.ExampleGroup(this._root);
      this._root.addGroup(group);
      func.apply(group, [new foounit.Context(group)]);
    }
    ...
    , build: function (group, level){
      ...
      for (var i = 0, ii = examples.length; i < ii; i++){
        ...
        var runner = new foounit.Runner(example, level);
        runners.push(runner);
      }
  ...
  safeset(foounit, 'suite.files', []);

  return foounit;
})();

if (foounit.env == 'node.js'){
  var node        = require(__dirname + '/node');
  foounit.add     = node.add || foounit.add;
  foounit.require = node.require;
  foounit.report  = node.report;
  foounit.puts('test');
  foounit.run     = node.run;
  foounit.exports = node.exports;
  foounit.files   = node.files;
